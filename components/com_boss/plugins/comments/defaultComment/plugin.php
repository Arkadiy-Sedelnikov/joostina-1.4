	...
	if ($conf->allow_comments == 1) {
		...
		if ($conf->secure_comment == 1) {
			...
			if ($captcha_keystring !== $captcha) {
				$link = sefRelToAbs("index.php?option=com_boss&amp;task=show_content&amp;&contentid=" . $row->contentid . "&amp;directory=$directory&amp;Itemid=$itemid");
		...
		if (($my->id == "0" && $conf->allow_unregisered_comment == 0)) {
			mosRedirect(sefRelToAbs("index.php?option=com_boss&amp;task=show_content&amp;contentid=" . $row->contentid . "&amp;directory=$directory&amp;catid=$catid&amp;Itemid=$itemid"), "");
			return;
		}
