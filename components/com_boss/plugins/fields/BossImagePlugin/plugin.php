...
class BossImagePlugin{
			...
			if(isset($_FILES["content_picture$i"])){
				if($_FILES["content_picture$i"]['size'] > $conf['max_image_size']->fieldvalue){
					if($isAdmin){
						...
						$url = JPATH_SITE . "/index.php?option=com_boss&amp;act=contents&amp;catid=&amp;directory=$directory";
					}
	...
	function onDelete($directory, $content){
		...
		while($isfile){
			...
			$pict = JPATH_BASE . "/images/boss/$directory/contents/" . $content->id . $ext_name . "_t.jpg";
			$pic = JPATH_BASE . "/images/boss/$directory/contents/" . $content->id . $ext_name . ".jpg";
