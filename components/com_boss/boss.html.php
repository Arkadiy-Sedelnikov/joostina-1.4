...
class boss_html{ //implements  bossUI {
	...
	<fieldset id="boss_fieldset">
		...
		<form action="<?php echo $target;?>" method="post" name="saveForm" enctype="multipart/form-data">
			...
			<input class='boss_required inputbox' size="50" id='name' type='text' name='name' maxlength='50'
				   value='<?php echo $user->name; ?>'/>
			<!-- name -->
			<br/>
			<!-- email -->
			<label for="email"><?php echo BOSS_FORM_EMAIL; ?></label>
			<input class='boss_required inputbox' size="50" id='email' type='text' name='email' maxlength='50'
			...
			<input class='inputbox' size="50" id='title' type='text' name='title' maxlength='50'
				   value="<?php echo BOSS_EMAIL_TITLE . htmlspecialchars(stripslashes($content->name), ENT_QUOTES); ?>"/>
			<!-- title -->
...
<div class="boss_maincat">
					...
					if($showImg == 1){
						if(file_exists(JPATH_BASE . "/images/boss/$directory/categories/" . $row->id . "cat.jpg"))
							echo '<a href="' . $link . '"><img class="imgcat" src="' . JPATH_SITE . '/images/boss/' . $directory . '/categories/' . $row->id . 'cat.jpg" alt="' . $row->name . '" /></a>'; else
	...
	function displayFilter($showName = 0){
		if($this->conf->filter == 'no')
			return; elseif(!is_file(JPATH_BASE . '/images/boss/' . $this->directory . '/plugins/filters/' . $this->conf->filter . '/plugin.php'))
			return;
	...
	function displayTags(){
		if(!empty($this->tags))
			return BOSS_TAGS . ': ' . $this->tags; else
			return false;
	}

	function displayListTags($content){
		if(isset($this->tags[$content->id]))
			return BOSS_TAGS . ': ' . $this->tags[$content->id]; else
			return false;
	}
...
*/
	...
	function displayCategoryImage($content){
		...
		if(($catid == 0) || (!file_exists(JPATH_BASE . '/images/boss/' . $directory . '/categories/' . $catid . 'cat_t.jpg')))
			echo '<img  class="imgheading" src="' . JPATH_SITE . '/templates/com_boss/' . $this->template_name . '/images/default.gif" alt="default" />'; else
		...
		foreach($fieldsgroup as $key => $f){
			...
			if($return->value != ""){
				...
				if($return->title != "" && $return->value != "" && $titleEnable == 1)
					echo "<span class=\"field_title\">" . $return->title . "</span> " . $divider . "<span class=\"field_value\">" . $return->value . "</span> "; else
					echo $return->value;
				echo $hrefEnd;
	...
	function displayCatImage(){
		...
		if(($catid == 0) || (!file_exists(JPATH_BASE . '/images/boss/' . $directory . '/categories/' . $catid . 'cat.jpg')))
			echo '<img  class="imgheading" src="/templates/com_boss/' . $this->template_name . '/images/default.gif" alt="default" />'; else
	...
	function displayCatTitle($showDefImg = 1){
		...
		if((($catid == 0) || (!file_exists(JPATH_BASE . '/images/boss/' . $directory . '/categories/' . $catid . 'cat_t.jpg'))) && $showDefImg == 1)
			echo '<img  class="imgheading" src="' . JPATH_SITE . '/templates/com_boss/' . $this->template_name . '/images/default_t.gif" alt="default" />'; else
			echo '<img  class="imgheading" src="' . JPATH_SITE . '/images/boss/' . $directory . '/categories/' . $catid . 'cat_t.jpg" alt="' . $name . '" />';
		echo $name;
	}
	...
	function displayOrderOption(){
		...
		<select name="order" size="1" onchange="jumpmenu('parent',this)">
			<option
				value="<?php echo sefRelToAbs($url . "&amp;order=0&amp;direction=" . $direction . "&amp;Itemid=" . $itemid);?>" <?php
				if($order == "0"){
					echo "selected='selected'";
				} ?>>
				<?php echo BOSS_DATE; ?>
			...
			if($s->sort == 1){
				?>
				<option
					value="<?php echo sefRelToAbs($url . "&amp;order=" . $s->fieldid . "&amp;direction=" . $direction . "&amp;Itemid=" . $itemid);?>" <?php
					if($order == $s->fieldid){
						echo "selected='selected'";
					} ?>>
	...
	function displayPathway(){
		...
		for($i = $nb - 1; $i > 0; $i--){
			...
			if(file_exists($filenamearrow))
				$pathway .= ' <img src="' . JPATH_SITE . '/templates/' . $cur_template . '/images/arrow.png" alt="arrow" /> '; else
				$pathway .= ' <img src="' . JPATH_SITE . '/templates/com_boss/' . $this->template_name . '/images/arrow.png" alt="arrow" /> ';
		}

		if($task == 'show_content')
			$pathway .= '<a href="' . $paths[0]->link . '">' . $paths[0]->text . '</a>'; else
	...
	<select name='category_choose' onchange="jumpmenu('parent',this)">
		<option
			value="<?php echo sefRelToAbs("index.php?option=com_boss&amp;task=search&amp;catid=0&amp;directory=$directory&amp;Itemid=$itemid"); ?>" <?php if($catid == 0)
	...
	<select class='boss_required inputbox' name='category' mosreq='1' mosLabel="<?php echo BOSS_FORM_CATEGORY; ?>">
		...
		if((@$content_id) && ($content_id != ""))
			$linkoption = "&amp;task=$task&amp;contentid=$content_id"; else
	...
	<select class='boss_required inputbox' name='content_types_choose' onchange="jumpmenu('parent',this)">
		...
		?>
		<option
			value='<?php echo sefRelToAbs("index.php?option=com_boss$linkoption&amp;content_types=0&amp;directory=" . $this->directory . "&amp;Itemid=$itemid"); ?>'><?php echo BOSS_SELECT_CONTENT_TYPE2; ?></option>
		<?php
		foreach($content_types as $row){
			...
			?>
			<option
				value="<?php echo sefRelToAbs("index.php?option=com_boss$linkoption&amp;content_types=" . $row->id . "&amp;directory=" . $this->directory . "&amp;Itemid=$itemid"); ?>"
	...
	function displayFormBegin($mode = "write"){
		...
		if($mode == "write")
			$target = sefRelToAbs("index.php?option=com_boss&amp;task=save_content&amp;directory=$directory&amp;Itemid=" . $itemid); else
	...
	function displayAccountCreationFields(){
		...
		if(($conf->submission_type == 0) && ($my->id == 0)){
			...
			if(isset($content->firstname))
				$firstname = $content->firstname; else
				$firstname = "";

			if(isset($content->middlename))
				$middlename = $content->middlename; else
				$middlename = "";
	...
	function displayWriteLink(){
		...
		if(!isset($catid))
			$link_write_content = sefRelToAbs("index.php?option=com_boss&amp;task=write_content&amp;directory=$directory&amp;Itemid=$itemid"); else
