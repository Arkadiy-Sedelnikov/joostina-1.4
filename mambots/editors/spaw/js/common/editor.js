...
SpawEditor.prototype.onLoadHookup = function () {
    ...
    if (window.onload != null) {
        var rgld = /\{([^}]+)/;
        var xx = rgld.exec(window.onload.toString());
        spaw_tmpstr = xx[1];
    }
            ...
            if (chnode.getAttribute("__spawprocessed") == null) // workaround to prevent elements from doubling
            {
                ...
                for (var j = 0; j < chnode.attributes.length; j++) {
                    ...
                    if (attr.nodeValue != null
                        ...
                        var attrnm = attr.nodeName.toLowerCase();
                        var tgnm = chnode.tagName.toLowerCase();
                        if (attrval == null)
                            attrval = chnode.getAttribute(attr.nodeName, 0);
                        if (tgnm != 'font') {
                            if (attrnm != 'class' && attrnm != 'style' && attrnm != 'href' && attrnm != 'src'
                                && attrnm != 'shape' && attrnm != 'coords' // coords and shape attributes are lost in area tag in IE
                                && attrnm != 'type' && attrnm != 'value' && attrnm != 'enctype') // type and value get lost on IE in input tags
                ...
                if (chnode.tagName.toLowerCase() != "script" && chnode.tagName.toLowerCase() != "style") {
                    ...
                    //replace bold and italic style with single tag (add by rav)
                    var re_bold = /^\W*style\W+font-weight\W+bold\W+$/i;
                    var re_italic = /^\W*style\W+font-style\W+italic\W+$/i;
                    if (tag_name == 'span') {
                        if (re_bold.test(attr_str)) {
                            tag_name = 'strong';
                            attr_str = '';
                        } else if (re_italic.test(attr_str)) {
                            tag_name = 'em';
                            attr_str = '';
                        }
                    }
