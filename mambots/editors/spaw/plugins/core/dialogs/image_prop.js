...
SpawImagePropDialog.init = function() 
{
  ...
  if (iProps)
  {
    ...
    if (iProps.style.width || iProps.width) {
      var wd = iProps.style.width ? iProps.style.width : iProps.width;
      if (isNaN(wd) && wd.indexOf("px") != -1)
        wd = wd.substring(0, wd.length-2);
      document.getElementById('cwidth').value = wd;
    }
    if (iProps.style.height || iProps.height) {
      var ht = iProps.style.height ? iProps.style.height : iProps.height;
	...
	var re_pd = /^\D*(\d+)\D*\s(\d+)/i;
	var pd_val = 0;
	if (iProps.style.margin && (pd_val = re_pd.exec(iProps.style.margin))) {
		document.getElementById('cvspace').value = pd_val[1];
		document.getElementById('chspace').value = pd_val[2];
	} else {
...
SpawImagePropDialog.okClick = function() {
  ...
  if (SpawImagePropDialog.validateParams())    
  {
	...
	tt = document.getElementById('cvspace').value;
	tt = !tt || tt == '' ? 0 : tt + "px";
	tx = document.getElementById('chspace').value;
	tx = !tx || tx == '' ? 0 : tx + "px";
	iProps.style.margin = (tt!=0 || tx!=0) ? tt + " " + tx : '';
	if(tt!=0 || tx!=0)
		iProps.style.padding = '';
	
	iProps.removeAttribute("height");
	tt = document.getElementById('cheight').value;
	iProps.style.height = (!tt || tt == '' || tt == 0) ? null : tt + "px"; 
	iProps.removeAttribute("width");
	tt = document.getElementById('cwidth').value;
	iProps.style.width = (!tt || tt == '' || tt == 0) ? null : tt + "px"; 
...
{
  if (document.getElementById("cproportions").checked && SpawImagePropDialog.aspectRatio != 0 && !isNaN(document.getElementById('cheight').value))
  {
