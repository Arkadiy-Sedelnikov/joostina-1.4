...
class SefJoossef implements JSefModel{
	...
	public static function getUrlToSef($link){
		...
		if((JSef::$cfg_frontpage AND stripos($link, 'option=com_frontpage') > 0 AND !(stripos($link, 'limit'))) OR $link == 'index.php'){
			$link = '';
		}
		...
		if(isset($url['query'])){
			...
			foreach($parts as $key => $value){
				...
				if($key != 'option')
					...
					$option = $key . self::separator . $value . '/';
			}
		}
