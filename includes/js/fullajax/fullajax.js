    ...
    (function (B) {
        B.extend(B, {version:"SRAX v1.0.3 build 1", TYPE:"full", Default:{prefix:"ax", sprt:":", loader:"loading", loader2:"loading2", loaderSufix:"_loading", DEBUG_AJAX:0, DEBUG_SCRIPT:0, DEBUG_LINK:0, DEBUG_STYLE:0, USE_FILTER_WRAP:1, NO_HISTORY:0, USE_HISTORY_CACHE:1, LENGTH_HISTORY_CACHE:100, LINK_REPEAT:0, USE_SCRIPT_CACHE:1, SCRIPT_SRC_REPEAT_APPLY:1, SCRIPT_NOAX:0, RELATIVE_CORRECTION:0, OVERWRITE:0, model2Marker:{ax:"<!-- :ax:", begin:":begin: //-->", end:":end: //-->"}, HAX_AUTO_DESTROY:0, HAX_ANTICACHE:0, DAX_AUTO_DESTROY:0, DAX_ANTICACHE:0, CHARSET:"UTF-8"}, debug:function (J, G) {
            ...
            if (K.indexOf("SRAX.init()") > -1) {
                K = '<script type="text/javascript"><\/script>'
            }
                    ...
                    if (W.src) {
                        ...
                        if (J > -1) {
                            ...
                            if ((L == null || (L != "false" && L != "0")) && C.SCRIPT_SRC_REPEAT_APPLY) {
                                ...
                                W = B.makeScript('<script type="text/javascript">//no repeat ' + W.src + "<\/script>")
                            }
                        } else {
                            try {
            ...
            var I = F ? J.responseText : "", G = B.makeScript('<script type="text/javascript">' + I + "<\/script>");
            G.place = D.place;
            G.id = D.id ? D.id : K;
            ...
            if (B.LoadUnloadContainer[F].onload) {
                B.parsingText({id:F, url:D, text:'<script id="' + E("script" + C.sprt + "temp") + '" type="text/javascript">' + B.LoadUnloadContainer[F].onload + "<\/script>", add:1})
            }
            if (B.isCOL) {
                window._onload()
            }
            ...
            if (typeof P == "string") {
                ...
                hax({id:Q.id, url:P, html:"<body onload=\"SRAX.get('" + Q.id + '\').cb()"><script type="text/javascript" src="' + P + '"' + (F ? " " + E("noax") + '="1"' : "") + (N ? " " + E("nocache") + '="1"' : "") + (T ? " " + E("noblock") + '="1"' : "") + "><\/script></body>", nohistory:1, storage:L});
                return
            }
            ...
            try {
                if (B.browser.safari) {
                    window._evalCode = F;
                    new B.addScript(B.makeScript('<script type="text/javascript">eval(window._evalCode)<\/script>'))
                } else {
            ...
            if (D.nodeName == "FORM") {
                if (D.attributes.action) {
                    G = D.attributes.action.nodeValue
                }
                if (!G) {
                    G = location.href
                }
            ...
            var K = location.protocol, L = location.host, I = K + "//" + L + location.pathname + location.search + "#", F = G.nodeName == "FORM" ? (G.attributes.action ? G.attributes.action.nodeValue : 0) : G.href;
            ...
            G.options = O;
            G.iswrapped = 1;
            var M = document.createAttribute("iswrapped");
            M.nodeValue = 1;
            ...
            if (!O.overwrite && !C.OVERWRITE) {
                if (B.browser.msie) {
                    if (G[F]) {
                        var N = document.createAttribute(H);
